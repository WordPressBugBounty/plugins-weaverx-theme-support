<?php
// File refactored: 2026-02-27 - wvrx-ts-runtime-lib
if (!defined('ABSPATH')) {
    exit;
...
function wvrx_ts_help_link(string $ref, string $label): void
{
	$t_dir   = wvrx_ts_plugins_url('/help/' . $ref, '');
	$pp_help = sprintf(
		'<a style="text-decoration:none;" href="%s" target="_blank" title="%s"><span style="color:red; vertical-align: middle; margin-left:.25em;" class="dashicons dashicons-editor-help"></span></a>',
		esc_url($t_dir),
		esc_attr($label)
	);
...
function wvrx_ts_fix_short($prefix, string $msg): void
{
	if (!$prefix) {
...
function wvrx_ts_theme_support_addon(): void
{
	...
	<div class="a-plus">
		<p><strong style="font-size:110%;"><?php
				...
				echo wp_kses_post(' (V ' . WVRX_TS_VERSION . ')'); ?></strong><br/>
...
Click the<span style="color:red; vertical-align: middle; margin-left:.25em;" class="dashicons dashicons-editor-help"></span> button to open help entry.', 'weaverx-theme-support' /*adm*/)); ?>
		...
		<h3><?php esc_html_e('Shortcodes', 'weaverx-theme-support' /*adm*/); ?></h3>
		<ul>
		...
		<form enctype="multipart/form-data" name='toggle_shortcode' action="<?php echo wp_kses_post($req_uri); ?>"
			  method='post'>

			<?php
			if ($is_xtreme) {
				$button = $prefix ? esc_html__("Remove 'wvrx_' prefix from shortcode names: [ bloginfo ], etc.", 'weaverx-theme-support') : esc_html__("Add 'wvrx_' to shortcode names: [ wvrx_bloginfo ], etc.", 'weaverx-theme-support');
				?>
				<div style="clear:both;"></div>
				<span class='submit'><input class="button-primary" name="toggle_shortcode_prefix" type="submit"
											value="<?php echo esc_attr($button); ?>"/></span>
				<br/><small> <?php esc_html_e("To avoid conflicts with other plugins, you can add a 'wvrx_' prefix to these shortcodes.", 'weaverx-theme-support' /*adm*/); ?> </small>
		...
		<?php if ($is_xtreme) { ?>
			...
			<p> <?php esc_html_e("Click the following button to produce a list of links to all pages and posts that have Per Page or Per Post settings.", 'weaverx-theme-support' /*adm*/); ?></p>
			<div style="clear:both;"></div>
			<form enctype="multipart/form-data" name='toggle_shortcode' action="<?php echo esc_url($req_uri); ?>"
				  method='post'>
				<span class='submit'><input class="button-primary" name="show_per_page_report" type="submit"
											value="<?php esc_attr_e('Show Pages and Posts with Per Page/Post Settings', 'weaverx-theme-support'); ?>"/></span>
...
{
	echo wp_kses_post("<!-- Weaver Xtreme Theme Support " . WVRX_TS_VERSION . " --> ");
}
...
function weaverx_ts_get_sysinfo(): string
{
	...
	$theme     = $theme_obj->Name . ' (' . $theme_obj->Version . ')';
	...
	$return .= "\n" . '        -- Server Configuration --' . "\n\n";
	$os = function_exists('php_uname') ? php_uname('s') : 'Unavailable on your version of PHP.';
	...
	$return .= 'Server Software:          ' . $server_software . "\n";
	...
	if (is_multisite()) {
		...
		$active_net  = get_site_option('active_sitewide_plugins', []);
		foreach ($net_plugins as $path) {
			$base = plugin_basename($path);
			if (isset($active_net[$base])) {
...
/*
 *
 * function myplugin_get_filesystem() {
...
$fs = myplugin_get_filesystem();
if ( ! $fs ) {
	// fallback / bail
}

$fs->put_contents( $path, $data, FS_CHMOD_FILE );
 */

function weaverx_ts_write_to_upload(string $filename, string $output): bool
{
	global $wp_filesystem;
	...
	$dir = trailingslashit($upload_dir['basedir']) . WEAVERX_SUBTHEMES_DIR . DIRECTORY_SEPARATOR;
	...
	if (!$wp_filesystem->is_writable($dir)) {
		...
		weaverx_f_file_access_fail(esc_html__('Directory not writable to save editor style file. Please install and activate the Weaver Xtreme Theme Support plugin. Directory: ', 'weaverx-theme-support') . $dir);
		return false;
	}

	$wp_filesystem->put_contents($dir . $filename, $output, 0744);
	return true;
}

function weaverx_ts_altwrite_to_upload(string $filename, string $output): bool
{
	$upload_dir = wp_upload_dir();
	$dir = trailingslashit($upload_dir['basedir']) . WEAVERX_SUBTHEMES_DIR . DIRECTORY_SEPARATOR;

	if (!wp_mkdir_p($dir)) {
		weaverx_f_file_access_fail(esc_html__('Directory not writable to save editor style file. You will have to check with your hosting company to have your installation fixed to allow directories to be created. Directory: ', 'weaverx-theme-support' /*adm*/) . $dir);
		return false;
	}
    ...
    if ( ! WP_Filesystem() ) {
        weaverx_f_file_access_fail(
                esc_html__(
                        'Unable to initialize WordPress filesystem. You may need to configure your server credentials.',
...
// Write file
    if ( ! $wp_filesystem->put_contents( $file_path, $output, FS_CHMOD_FILE ) ) {
        weaverx_f_file_access_fail(
                esc_html__(
                        'Unable to save editor style file. You will have to check with your hosting company to have your installation fixed to allow files to be created.',
                        'weaverx-theme-support'
                ) . $dir
        );
        return false;
